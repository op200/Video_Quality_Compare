...
if os_type == 'Windows':
    config_dir = os.path.join(os.getenv('APPDATA'), 'VQC')
...
input_video_path_1, input_video_path_2, input_video_path_now = str, str, str
...
#输入路径 初始化
def submit_path(_):
    global input_video_path_1,input_video_path_2,input_video_path_now,scale,main_rendering_Cap,frame_count,frame_height,frame_width,new_frame_height,new_frame_width,fps,frame_now
    ...
    if ret:
        ...
        algorithm.data = [None]*frame_count
        ...
        def _long_text_cut(text):
            ...
            if len(text)>149:
                text = text[0:149]+"\n"+text[149:]
            return text
        video_path_review_Label.config(text=_long_text_cut(input_video_1_Entry.get())+'\n'+_long_text_cut(input_video_2_Entry.get()))
...
def change_video_preview():
    ...
    jump_to_frame()
change_video_preview_Button = ttk.Button(input_video_Frame, text="切换预览视频", width=12, command=change_video_preview)
...
cmp_select_Combobox = ttk.Combobox(cmp_set_Frame, width=4, values=['ssim','psnr','vmaf'])
cmp_select_Combobox.state(['readonly'])
cmp_select_Combobox.set(config.get("DEFAULT","cmp_type"))
cmp_select_Combobox.grid(row=0,column=0)

start_cmp_Frame = tk.Frame(cmp_set_Frame)
start_cmp_Frame.grid(row=1,column=0,pady=15)

max_min_Frame = ttk.Frame(start_cmp_Frame)
max_min_Frame.grid(row=0,column=0)

max_data_Label = ttk.Label(max_min_Frame,width=22,anchor='center')
max_data_Label.grid(row=0,column=0)
max_data_Label.config(state=tk.DISABLED)

min_data_Label = ttk.Label(max_min_Frame,width=22,anchor='center')
min_data_Label.grid(row=0,column=1)
min_data_Label.config(state=tk.DISABLED)

show_data_Label = ttk.Label(start_cmp_Frame,width=44,anchor='center')
...
class algorithm:
    ...
    @staticmethod
    def find_line_ssim(line):
        return re.findall("[\d\.]+", re.search("Y(.*)\(",line).group(1))

    @staticmethod
    def find_line_psnr(line):
        return re.findall("[\d\.]+", re.search("psnr_avg(.+)$",line).group(1))
    
    find_line = find_line_ssim


    @staticmethod
    def read_file_ssim_psnr(file):
        for line in file:
    ...
    @staticmethod
    def get_data_vmaf():
        algorithm.min_data = algorithm.data[0]
        algorithm.max_data = algorithm.data[0]
        for v in algorithm.data:
            if v > algorithm.max_data:
                algorithm.max_data = v
            if v < algorithm.min_data:
                algorithm.min_data = v
    ...
    def listen_data_ssim():
        ...
        if v:
            ...
            show_data_Label.config(
                ...
                foreground=f"#{round(255*(1-color_percent)):02X}{round(255*color_percent):02X}00")

    def listen_data_psnr():
        v = algorithm.data[frame_now]
        if v:
            dif = algorithm.max_data-algorithm.min_data
            if dif==0:
    ...
    def change_algorithm():
        ...
        match algorithm.algorithm:
            case "ssim":
                ...
                algorithm.read_file = algorithm.read_file_ssim_psnr
            ...
            case "psnr":
                ...
                algorithm.read_file = algorithm.read_file_ssim_psnr
                algorithm.get_data = algorithm.get_data_psnr
                algorithm.listen_data = algorithm.listen_data_psnr
                algorithm.get_plt = algorithm.get_plt_psnr
            case "vmaf":
...
def Thread_encoding():
    ...
    max_data_Label.config(state=tk.NORMAL,
                          text=f"Max:{algorithm.max_data}", foreground='green')
    min_data_Label.config(state=tk.NORMAL,
                          text=f"Min:{algorithm.min_data}", foreground='red')
