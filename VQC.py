...
scale, cut_scale, frame_height, frame_width, new_frame_height, new_frame_width = False, False, int,int,int,int
...
def jump_to_frame():
    ...
    try:
        ...
        if scale:
            if cut_scale:
                frame = frame[cut_scale_y : cut_scale_y+new_frame_height, cut_scale_x : cut_scale_x+new_frame_width]
            else:
...
def video_review_on_drag_start(event):
    global video_review_on_drag_start_x,video_review_on_drag_start_y,cut_scale_x_start,cut_scale_y_start
...
def change_video_scale():
    ...
    if cut_scale:
        ...
        cut_scale = True
    jump_to_frame()
change_video_scale_Button = ttk.Button(input_video_Frame, text="切换缩放", width=8, command=change_video_scale)
...
cmp_select_Combobox = ttk.Combobox(cmp_set_Frame, width=4, values=['ssim','psnr','vmaf','mse'])
...
max_data_Label = ttk.Label(extreme_value_Frame,width=14,anchor='center',foreground='#00BB00')
max_data_Label.grid(row=0,column=0)
max_data_Label.config(state=tk.DISABLED)

avg_data_Label = ttk.Label(extreme_value_Frame,width=14,anchor='center',foreground='#BBBB00')
avg_data_Label.grid(row=0,column=1)
avg_data_Label.config(state=tk.DISABLED)

min_data_Label = ttk.Label(extreme_value_Frame,width=14,anchor='center',foreground='#BB0000')
...
class algorithm:
    ...
    def find_line_ssim(line):
        return re.findall(r"[\d\.]+", re.search(r"Y(.*)\(",line).group(1))

    @staticmethod
    def find_line_psnr(line):
        return re.findall(r"[\d\.]+", re.search(r"psnr_avg(.+)$",line).group(1))

    @staticmethod
    def find_line_mse(line):
        return re.findall(r"[\d\.]+", re.search(r"mse_avg(.+)psnr_avg",line).group(1))
    ...
    def read_file_vmaf(file):
        for line in file:
            ...
            if text:
                ...
                text = re.search(r'<metric name="vmaf" min="([\d\.]+)" max="([\d\.]+)" mean="([\d\.]+)"', line)
                if text:
